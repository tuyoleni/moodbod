...
export function Navbar({ isCTASectionVisible }: Props) {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 flex items-start justify-between p-8">
            {/* Mobile Menu Button */}
            <button
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                className="md:hidden relative z-50 py-2 "
            ...
            {/* Mobile Navigation */}
            {isMenuOpen && (
                <div className="md:hidden fixed inset-0 backdrop-blur-md" onClick={() => setIsMenuOpen(false)}>
                    <div className="pt-24 px-8" onClick={e => e.stopPropagation()}>
                        {navLinks.map((link, index) => (
                            <button
                                key={link.id}
                                onClick={() => handleScrollTo(link.id)}
